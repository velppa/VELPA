...
(org-export-define-backend 'pollen
  `((bold                . ,(ox-pollen--block "b"))
    ...
    ;; inline-code blocks
    (code                . ,(ox-pollen--value-block "sans"))
...
(defun ox-pollen-headline (headline contents info)
  "Transcode headline OBJ into h1, h2, h3..."
  (let ((level (org-export-get-relative-level headline info))
        (id (replace-regexp-in-string
             (rx (or "\"" "=" "~" " ")) "-"
             (downcase (org-element-property :raw-value headline))))
        (value (org-export-data (org-element-property :title headline) info)))
    (unless (org-element-property :footnote-section-p headline)
      (let ((value (plist-get (cl-second headline) :raw-value)))
        (cond
         ((= level 1) (format "◊section{\n◊heading[#:id \"%s\"]{%s}\n%s}" id value (or contents "")))
         ((= level 2) (format "◊subheading[#:id \"%s\"]{%s}\n%s" id value (or contents "")))
         (format "◊p{%s}\n%s" value (or contents "")))))))

(defun ox-pollen-src-block (obj &rest _)
  (format "◊src[#:class \"lang-%s\"]{\n%s\n}"
...
(defun ox-pollen-export-to-pollen (filename)
