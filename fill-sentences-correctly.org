...
With =fill-sentences-correctly-mode= enabled, filling paragraphs and
regions will result in the first result, but you'll still be able to
use keybindings like ~M-a~ and ~M-e~ to navigate sentences with a single
space after a period.  This is very convinient.  The package will not
turn single spaces into double spaces in existing paragraphs.
